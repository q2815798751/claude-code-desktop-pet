...
- **告警**：有**未读**告警时圆环右上出现红色角标（数量），底部显示最高优先级告警的**主题卡片**：
  第一行标题、第二行明细，按级别着色（warn 琥珀 / crit 红），最多三行。
  它是**实底卡片**（`--bg1→--bg2` 渐变 + 左侧强调色边），不是直接压在 GIF 上的裸字——
  球窗口只有 150×170，文字必然与圆环和桌宠重叠，实底是让它可读的前提。
  卡片 `pointer-events:none`，不影响拖动；角标才是点击目标。
