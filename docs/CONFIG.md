...
| `GET /api/state` | `{state, since, tool, long, lastActivity, ageMs, claude, hooked, term, termAlive, forced, lastAction, error, shutdown, runtime, usage, alerts, ts}` |
...
- **告警**：有**未读**告警时圆环右上出现红色角标（数量），底部显示最高优先级告警的**标题 + 明细两行**
  （球窗口只有 150×170，一行放不下；见 `pet.html` 的 `.ball-alert`）。
...
- 状态行下方是**常驻用量瘦条**：`↑输入 ↓输出 CTX ▓▓░ 百分比`；CTX 超过 `ctxWarnPct`/`ctxCritPct` 时转琥珀/红。
  点瘦条或 `▤` 切到用量页。
- **用量页**（占据中间桌宠区，桌宠隐藏）：本次 / 今日 / 累计 三个 tab，显示输入、输出、缓存命中率、API 调用次数；
  「本次」额外显示上下文占用条；下方是近 7 日柱状（末根为今天，零值日显示为灰色基线）。
  底部：告警开关 + 字号 `A−` / `A+`。
- **告警气泡**：从 `现在:` 上方往上堆叠，最多 3 条；8 秒后自动消失（底部倒计时条），点卡片或 `×` 立即关闭。
